<template >
  ...
  <div style="border:8px solid #EDEDED;border-top:0px;border-bottom:0px ;">
    <!--<div>-->
    <mt-loadmore style="background: #EDEDED" :autoFill=false :top-method="loadTop" :bottomPullText="pullup" @top-status-change="handleTopChange" :bottom-all-loaded="allLoaded" ref="loadmore" :bottom-method="loadbottom" @bottom-status-change="handlebottomChange">
      <div id="about">
        <div class="aboutMeMsg" v-for="(item,index) in list">
          <mt-cell-swipe :class="item.id" :right="[
                      {
                        content: '<div>×</div>',
                        handler:function(){
            ...
            <ul>
              ...
              <li>
                <span style="flex:100%">{{item.companyName}}</span>
              </li>
              <li>
                <span style="flex:100%">{{item.description}}</span>
              </li>
